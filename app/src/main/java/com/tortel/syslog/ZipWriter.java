...
public class ZipWriter {
	...
	private JarOutputStream jWriter;
	private File[] files;
	
	public ZipWriter(String path, String zName) throws IOException, NoFilesException{
		outPath = path;
		...
		if(files == null || files.length == 0){
			Log.e("SysLog", "Error - no files to zip/jar.");
			throw new NoFilesException();
		}
		for(File cur: files){
			Log.v("SysLog", "File to be zipped/jarred: "+cur.getPath());
		}
		
		zWriter = new ZipOutputStream(new FileOutputStream(outPath+zName));
		jWriter = new JarOutputStream(new FileOutputStream(outPath+zName));
	}
    ...
    public void createZip() throws IOException, LowSpaceException {
        try {
            for (int i = 0; i < files.length; i++) {
                ...
                JarEntry j=new JarEntry(cur.getName());
                j.setSize(cur.length());
                jWriter.putNextEntry(j);
